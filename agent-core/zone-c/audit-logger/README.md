...
| Owner cannot quietly remove the triggers | `900_ddl_guard.sql` event triggers reject `ALTER TABLE/FUNCTION/...`, every `ALTER SCHEMA`, `CREATE RULE/TRIGGER` on it, and `DROP` of anything in schema `audit` for non-superusers (schema `sharp` has its own guard in sharp-gate) |
