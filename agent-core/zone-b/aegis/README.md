...
| `AEGIS_SUPERVISOR_HEARTBEAT_FILE` | no (recommended; set in compose) | written after every completed probe step (unix ms, atomic write); read by `aegis supervisor-healthcheck` |
| `AEGIS_SUPERVISOR_HEARTBEAT_MAX_AGE_MS` | no | healthcheck staleness limit, default 10000, in [1000, 60000] |
...
policy and alert on restarts.

**Supervising the Supervisor (ALI-163).** A restart policy only covers a
Supervisor that exits. One that is alive but stuck would silently switch the
liveness backstop off. So after every completed step (whatever Aegis's state:
watching a dead Aegis or having tripped HARD is the Supervisor doing its job) it
writes a heartbeat to `AEGIS_SUPERVISOR_HEARTBEAT_FILE`, and
`aegis supervisor-healthcheck` exits 0 only if that beat is fresh (1 if missing,
unparseable, too old or far in the future; 2 if misconfigured). Compose runs the
Supervisor with this healthcheck, replacing the image's `aegis healthcheck` (a
TCP connect to the Aegis listen port, which does not apply here), and a tmpfs for
the file. Alert on the container going unhealthy or restarting: that alerting is
deployment-side and not provided here. Independently, execution-motor treats a
lost Aegis state stream as HARD and cancels open orders (ALI-162), so a dead
Aegis halts execution even if the Supervisor is down. Unknown subcommands exit 2
instead of starting the server. Tests: `src/supervisor/`,
